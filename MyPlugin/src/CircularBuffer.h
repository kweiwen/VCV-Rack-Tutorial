...
public:
    ...
    float readBuffer(float delayInFractionalSamples, bool interpolate = true);
...
private:
    std::unique_ptr<float[]> mBuffer = nullptr;
    unsigned int mWriteIndex;
    unsigned int mBufferLength;
    unsigned int mWrapMask;
};

inline void CircularBuffer::createCircularBuffer(unsigned int input)
{
    // --- reset the to top
    mWriteIndex = 0;
    ...
    mBuffer.reset(new float[mBufferLength]);
    // --- clean the value inside mBuffer
    flushBuffer();
}

inline void CircularBuffer::flushBuffer()
{
    for (unsigned int i = 0; i < mBufferLength; i++)
    {
        mBuffer[i] = 0;
    }
}

inline void CircularBuffer::writeBuffer(float input)
{
    mBuffer[mWriteIndex++] = input;
    mWriteIndex &= mWrapMask;
}

inline float CircularBuffer::readBuffer(int delayInSamples)
{
...
inline float CircularBuffer::readBuffer(float delayInFractionalSamples, bool interpolate /*= true*/)
{
...
inline float CircularBuffer::doLinearInterpolation(float delayInFractionalSamples)
{
    float y1 = readBuffer((int)delayInFractionalSamples);
...
inline float CircularBuffer::doHermitInterpolation(float delayInFractionalSamples)
{
...
inline float CircularBuffer::doLagrangeInterpolation(float delayInFractionalSamples)
{
    int n = 4;
    int index = (int)delayInFractionalSamples;
    float x[4] = { static_cast<float>(index - 1), static_cast<float>(index), static_cast<float>(index + 1), static_cast<float>(index + 2) };
